"""
Prompt templates for PR reviewing.
...
zero_shot_prompt = ChatPromptTemplate.from_messages([
    ...
    (
        "human",
        "Review the following GitHub Pull Request diff. Point out bugs, "
...
few_shot_prompt = ChatPromptTemplate.from_messages([
    ...
    (
        ...
        "test.\n\n"
        "Example 2:\n- Code style: inconsistent naming `myVar` vs "
        "`my_var`.\n"
...
cot_prompt = ChatPromptTemplate.from_messages([
    ...
    (
        ...
        "Diff:\n{diff}\n\n"
        "IMPORTANT: Provide only the final review in the 'Final Review' "
        "section at the end."
    )
])
...
self_consistency_prompt = ChatPromptTemplate.from_messages([
    ...
    (
        ...
        "3) Return the best review and a 1-line reason why you picked "
        "it.\n\nDiff:\n{diff}"
    )
])
...
reflection_prompt = ChatPromptTemplate.from_messages([
    (
        "system",
        "You are a senior engineer. Produce a review, critique it, then "
        "refine it."
    ),
    (
        "human",
...
meta_prompt = ChatPromptTemplate.from_messages([
    ...
    (
        ...
        "when possible). Keep it concise and prioritize items by "
        "severity.\n\nDiff:\n{diff}"
    )
])
