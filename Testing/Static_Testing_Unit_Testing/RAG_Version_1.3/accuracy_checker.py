"""
Accuracy checker module for evaluating PR reviews using LLM scoring and heuristics.
"""
...
evaluator_prompt = ChatPromptTemplate.from_messages([
    ...
    (
        "human",
        (
            "You will evaluate a Pull Request review based on the diff, static analysis, "
            "and retrieved context provided.\n"
...
def meta_evaluate(diff: str, review: str, static_output: str, context: str):
    ...
    try:
        ...
        # LLM may raise unpredictable exceptions → allow broad exception
...
def heuristic_metrics(review: str):
    ...
    heur["bullet_points"] = len(
        re.findall(r"^\s*[-•*]\s+", review, flags=re.MULTILINE)
    )
    heur["mentions_bug"] = bool(
        re.search(r"\bbug\b|\berror\b|\bfail\b|\bissue\b", review, flags=re.I)
    )
    heur["mentions_suggest"] = bool(
        re.search(
